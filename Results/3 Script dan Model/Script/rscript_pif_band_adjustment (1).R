...
reference.file <- "L8RTP121060m_150718_geo_mask.tif"
ref <- stack(reference.file)
ref <- stack(ref[[2:7]])

#foreach(i = 1:length(dir.lst), packages = c("raster", "RStoolbox")) %dopar% {
for(i in 2:length(dir.lst)){
  setwd(dir.lst[i])
  data <- xmlParse("MTD_MSIL1C.xml")
  xml_data <- xmlToList(data)
  sen.type <- xml_data$General_Info$Product_Info$Datatake$SPACECRAFT_NAME
  list.sentinel2 <- list.files(getwd(), "_25.tif")
  list.sentinel2 <- list.sentinel2[!str_detect(list.sentinel2,pattern="calibrated")]
  list.sentinel2 <- list.sentinel2[!str_detect(list.sentinel2,pattern="area")]
  list.sentinel2 <- list.sentinel2[!str_detect(list.sentinel2,pattern="ovr")]  
  sen.uncal <- stack(list.sentinel2)
  name <- strsplit(basename(list.sentinel2), ".tif")[[1]][1]
  ...
  writeRaster(sen.pif1$img, paste0("bpa_calibrated_cor_", name, ".tif"), format= "GTiff", datatype = "FLT4S")
  #writeRaster(sen.pif2$img, paste0("bpa_calibrated_ed_", name, ".tif"), format= "GTiff", datatype = "FLT4S")
  recap <- cbind(rsq1a, rsq2a, pv1a)
  names(recap) <- c("rsq1", "adj.rsq1", "pvalue1")
  write.csv(recap, paste0("bpa_recap_pif_", name, ".csv"))
  writeRaster(sen.pif1$pifMap, paste0("bpa_pif1_area_cor_", name, ".tif"), format= "GTiff", datatype = "FLT4S")
