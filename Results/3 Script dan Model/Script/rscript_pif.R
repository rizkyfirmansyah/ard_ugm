...
list.sentinel2 <- list.files(list.dir.sen2, "Geo_T50NMG_20180920T021559_25_CM.tif$", full.names = T, recursive = F)
...
reference.file <- "L8RTP117059m_140418_geo_mask.tif"
...
for(j in 1:nlayers(sen.uncal)){
  ...
  file.calibrated <- list.files(getwd(), "calibrated_")
  file.pif <- list.files(getwd(), "pif")
  file.copy(file.calibrated,list.dir.sen2[i])
  file.remove(file.calibrated)
  file.copy(file.pif, list.dir.sen2[i])
  file.remove(file.pif)
}

stopCluster(cl)

list.cal <- list.files(list.dir.sen2, "calibrated_", full.names = T, recursive = F)
list.pif <- list.files(list.dir.sen2, "pif", full.names = T, recursive = F)
file.remove(list.pif)
file.remove(list.cal)


list.sentinel2 <- list.files(getwd(),"_CM.tif$", full.names = T, recursive = F)
for(i in 1:length(list.sentinel2)){
  sen.uncal <- stack(list.sentinel2[i])
  sen.uncal <- calc(sen.uncal, fun.maskrm)
  sen.uncal <- resample(sen.uncal, ref, "bilinear")
  name <- strsplit(basename(list.sentinel2[i]), ".tif")[[1]][1]
  print(paste0("start processing ", name))
  sen.pif1 <- pifMatch(sen.uncal, ref, method = "cor", quantile = 0.95, returnModels = T)
  for(j in 1:nlayers(sen.uncal)){
  ...
  writeRaster(sen.pif1$img, paste0("calibrated_cor_", name, ".tif"), format= "GTiff", datatype = "FLT4S")
  recap <- cbind(rsq1a, rsq2a, pv1a)
  names(recap) <- c("rsq1", "adj.rsq1", "pvalue1")
  write.csv(recap, paste0("recap_pif_", name, ".csv"))
  writeRaster(sen.pif1$pifMap, paste0("pif1_area_cor_", name, ".tif"), format= "GTiff", datatype = "FLT4S")
  file.calibrated <- list.files(getwd(), "calibrated_")
  file.pif <- list.files(getwd(), "pif")
